...
| `notebooks/06_survival_analysis.ipynb` | **Eksperimen terpisah** (tidak menggantikan model resmi): Cox Proportional Hazards per siklus pasang (bukan snapshot 30-harian) untuk mengurutkan PART mana yang waktunya paling dekat ke kerusakan, uji formal asumsi proportional-hazards, plus percobaan Random Survival Forest sebagai challenger (ditolak - C-index test lebih rendah dari Cox PH di semua kombinasi hyperparameter) | Kurva Kaplan-Meier, koefisien Cox, tabel C-index, uji asumsi (dilanggar signifikan), perbandingan RSF, dan kesimpulan jujur soal batasan/kegunaannya |
